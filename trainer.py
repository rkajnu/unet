...
parser.add_argument('--rotation_range', type=float, default=0.2, help='Rotation range for data augmentation')
parser.add_argument('--width_shift_range', type=float, default=0.05, help='Width shift range for data augmentation')
parser.add_argument('--height_shift_range', type=float, default=0.05, help='Height shift range for data augmentation')
...
parser.add_argument('--horizontal_flip', type=bool, default=True, help='Whether to apply horizontal flip for data augmentation')
...
model = unet()
model_checkpoint = tf.keras.callbacks.ModelCheckpoint('unet_membrane.keras', monitor='loss',verbose=1, save_best_only=True)
model.fit(myGene,steps_per_epoch=args.steps_per_epoch,epochs=args.epochs,callbacks=[model_checkpoint])
...
mlflow.data.from_numpy(features = samples_arr, targets = labels_arr, name ='sample_ds')
